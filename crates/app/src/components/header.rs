    ...
    view! {
        <header>
            <nav>
                ...
                " | "
                <A href=move || localized_path(locale_now.get(), td_string!(locale_now.get(), routes.contact_path))>
                ...
                <Show when=move || auth.ready.get() fallback=|| ()>
                    ...
                    <Show when=move || auth.logged_in() fallback=|| ()>
                        <A href=move || localized_path(locale_now.get(), td_string!(locale_now.get(), routes.protected_path))>
                            {t!(i18n, header.protected)}
                        </A>
                        " | "
                        <A href=move || localized_path(locale_now.get(), td_string!(locale_now.get(), routes.account_path))>{t!(i18n, header.account)}</A>
                        " | "
                        <Show when=move || auth.is_admin() fallback=|| ()>
                            <A href=move || localized_path(locale_now.get(), td_string!(locale_now.get(), routes.admin_path))>
                                {t!(i18n, header.admin)}
                            </A>
                            " | "
                        </Show>
                        <A href=move || localized_path(locale_now.get(), td_string!(locale_now.get(), routes.logout_path))>
